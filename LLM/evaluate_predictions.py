...
def main(model, evaluation_level):
    ...
    if "human_evaluation" in evaluation_level:
        different_level_valid_cnt = {"area_level": 0,
                                     "space_level": 0,
                                     "object_level": 0,
                                     "part_level": 0}
        IOU25 = {"area_level": 0,
        ...
        for single_info in human_evaluation_info:
            
            exp_level = single_info["evaluation_level"]
            scene_id = single_info["scene_id"]
            # scene_name = single_info["scene_name"]
            datasetname = single_info["datasetname"]
            exp_file_index = int(single_info["exp_file_index"])
            db_id = single_info["db_id"]
            
            # if exp_level == "area_level":
            #     print(exp_level, scene_id, single_info["new_referring_expressions"])

            path_gt = "../anywhere3d_datasets/anywhere3d_" + exp_level + "_aligned.xlsx"
            ...
            path_pred = f'./{model}/{exp_level}/{datasetname}-{scene_id}-{db_id}.json'
            # path_pred = f'./{model}/human_evaluation/{exp_level}-{datasetname}-{scene_id}-{db_id}.json'
            print(path_pred)
...
if __name__ == "__main__":
    main(model = "deepseek-r1-671b", evaluation_level = "object_level")
